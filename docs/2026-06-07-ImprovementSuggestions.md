...
- [x] **Add app `shortcuts` to [manifest.json](../manifest.json)** — Long-pressing the installed icon should offer quick jumps: **Today/Devotional**, **Sabbath School Lessons**, **Newsletter**, **Calendar**, **Give**. This is a few lines of JSON and directly serves the "I don't want to hunt for it" need.
- [x] **Cache today's devotional + lesson links for offline use** — `devotional-today.json` now uses a **network-first** strategy in [sw.js](../sw.js) (was stale-while-revalidate) so users always get today's devotional when online; the last cached copy is served when offline.
- [x] **Decide the PWA `start_url` deliberately** — `start_url` changed to `/today.html?source=pwa`; SW navigate handler updated to fall back to the cached page (ignoring query params) before serving the offline placeholder, so the installed app shows real content when offline.
- [x] **Custom "Add to Home Screen" prompt** — `initInstallPrompt()` added to [assets/js/main.js](../assets/js/main.js): captures `beforeinstallprompt`, shows a dismissible slide-up banner on the 2nd+ visit, respects `localStorage` dismissal, and fires the native install dialog on "Install" click.
- [x] **Per-platform splash/theme polish** — Added `<link rel="apple-touch-icon">` to all 11 PWA-enabled pages; split the manifest `"any maskable"` combined purpose into proper separate `"any"` and `"maskable"` entries; removed non-standard `media` property from manifest icons.
...
- [x] **Periodic Background Sync to refresh the devotional — evaluate only** — Limited browser support (Chrome/Edge Android only; no iOS Safari). Implemented option 2 instead: a `visibilitychange` listener in [assets/js/daily.js](../assets/js/daily.js) that re-runs `init()` when the user returns to the app after ≥ 5 minutes, keeping content fresh on all platforms including iOS with no permission prompts. The existing countdown `setInterval` is tracked via a module-level `countdownIntervalId` and cleared before each re-init to prevent duplicate tickers.
